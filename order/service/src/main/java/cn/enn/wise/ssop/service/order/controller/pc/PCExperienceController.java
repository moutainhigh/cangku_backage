...
@Slf4j
@RestController
@RequestMapping("/pc/experience")
@Api(value = "PC体验项目", tags = {"PC体验项目"})
public class PCExperienceController {
        ...
        orderGoodsList.forEach(c->{
            ...
            if(extraInformation!=null&&extraInformation!=""){
                ...
                if(c.getOrderId().equals(orderInfo.getOrderId())){
                    ...
                    if(map.containsKey("operator")){
                        JSONObject operator = JSONObject.parseObject(map.get("operator").toString());
                        OrderOperatorsListDTO orderOperatorsListDTO = JSONArray.toJavaObject(operator, OrderOperatorsListDTO.class);
